...
登录服务器后（root / sudo 权限），安装 Node.js 22，并在 `/opt/` 下克隆源码、构建发布：
...
# 2) 创建源码目录（放 /opt/ 下）并克隆代码
sudo mkdir -p /opt/dsh-desktop
sudo chown -R $USER:$USER /opt/dsh-desktop
cd /opt/dsh-desktop
git clone https://github.com/foxi-ui/dsh-desktop-community.git .

# 3) 创建站点发布目录（Nginx root）
...
# 5) 发布构建产物到站点目录
cp -a dist/* /var/www/dsh-desktop/
```

> **目录分工**：源码位于 `/opt/dsh-desktop`（git 检出 + 构建），发布产物位于
> `/var/www/dsh-desktop`（Nginx root），两者分离，更新互不影响。
...
# 出问题时恢复（源码在 /opt/dsh-desktop，不受影响）
...
| 图片/字体 404 | 确认构建产物已发布到 `/var/www/dsh-desktop` 根目录（`cp -a dist/* /var/www/dsh-desktop/`） |
| 证书快到期 | `sudo certbot renew --dry-run` 检查；timer 自动续期 |
| 构建失败 | 在 `/opt/dsh-desktop` 下执行；`npm ci` 报错先 `rm -rf node_modules package-lock.json && npm install` 重试（npm 平台依赖已知问题） |
